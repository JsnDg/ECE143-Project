...
'''
Function: for normalization of the data 
'''
...
def month_temp_barplots(normalize=True):
  '''
  Function: to generate the barplots demonstrating the realtionshop between month and temperature/normalized temp
  '''
  if normalize==True:
    ...
    t='normalized_temp'
  sns.barplot(x=fire['month'],y=fire[t], # demonstrate the probability distribution of two variables
           data = fire,   # display shades
           color = 'blue',  # set the color palatte
           )# the color change indicates the change of density
  plt.grid(linestyle = '--')
  plt.scatter(fire['month'], fire[t], s=5, alpha = 0.5, color = 'k', marker='+') #scatter
  sns.rugplot(fire['month'], color='g', axis='x',alpha = 0.5)
  sns.rugplot(fire[t], color='r', axis='y',alpha = 0.5)
  plt.show()

month_temp_barplots(normalize=True)

'''
create new variable/column: 
summer_fire, which only displays the fire in summer months
non-summer_fire, which only displays the fire in non-summer months
...
def time_firefrequency_barplots(month=True,day=True,figNo=2):
  '''
  Function: to generate barplots demonstrating the fire frequency against different time index
  : type month/day: bool
  : param month/day: time index can be month or day
  : type figNo: int
  : param figNo: number of plots

  For month bar plots, users can identify the fire frequency in different months
  For day bar plots, user can identify the fire frequency in different days(weekdays and weekends)

  '''
  assert month+day==figNo
  assert isinstance(month,bool)
  assert isinstance(day,bool)
  assert isinstance(figNo,int) and figNo>0
  if month==True and day==False:
    t=['Summer Fire Frequency'] # title
    y1=['summer_fire'] # partial bar plots: summer  
    y2=['non-summer_fire'] # partial bar plots :non-summer
  elif day==True and month==False:
    t=['Day Fire Frequency']
    y1=['weekday_fire'] # partial bar plots: weekdays
    y2=['weekend_fire'] # partial bar plots: weekends
  else:
    t=['Summer Fire Frequency','Day Fire Frequency']
    y1=['summer_fire','weekday_fire'] 
    y2=['non-summer_fire','weekend_fire']
  for i in range(figNo):
    plt.figure(i)
    plt.title(t[i], fontsize=14, position=(0.5,1.05))
    sns.barplot(x=fire['month'],y=fire[y1[i]], # demonstrate the probability distribution of two variables
           data = fire,   # display shades
           color = 'Red',  # set the color palatte
           )# the color change indicates the change of density
    sns.barplot(x=fire['month'],y=fire[y2[i]], # demonstrate the probability distribution of two variables
           data = fire,   # display shades
           color = 'Blue',  # set the color palatte
           )# the color change indicates the change of density
    plt.grid(linestyle = '--')
...
time_firefrequency_barplots(month=True,day=True,figNo=2)
